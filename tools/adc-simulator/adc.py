...
class TimestampedADC:
    ...
    class RawBuffer:
        ...
        STRUCT_FORMAT = f"@Q6H{30 * 6 - 6}H" # 1 PRU timestamp (nanoseconds; unsigned 64-bit), 6 ADC values (unsigned 16-bit), rest padding
        BUFSIZE = struct.calcsize(STRUCT_FORMAT)
        ...
        def __init__(
            ...
            assert TimestampedADC.RawBuffer.BUFSIZE == 368, "RawBuffer size must be 368 bytes"
        ...
        def __bytes__(self) -> bytes:
            b = struct.pack(
                ...
                self.timestamp_nsec,
                *self.channel_values,
                *([0] * max(0, 30 * 6 - len(self.channel_values)))
            )
            return b
