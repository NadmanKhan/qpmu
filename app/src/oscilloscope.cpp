...
Oscilloscope::Oscilloscope(QWidget *parent) : QWidget(parent)
{
    ...
    for (uint64_t i = 0; i < qpmu::CountSignals; ++i) {
...
void Oscilloscope::updateView()
{
    ...
    for (uint64_t i = 0; i < (uint64_t)samples.size(); ++i) {
        const auto &t = samples[i].timestampUsec;
        timeMin = std::min(timeMin, (int64_t)t);
        timeMax = std::max(timeMax, (int64_t)t);
        for (uint64_t j = 0; j < qpmu::CountSignals; ++j) {
